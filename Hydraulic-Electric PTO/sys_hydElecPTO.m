...
nonState.Tpm = (nonState.pmPumping/nonState.eta_m_m ...
...
for iWEC = 1:par.NumWECs
    ...
    dydt(par.iy.theta(iWEC)) = dydt_WEC(1,iWEC); % angular velocity
    dydt(par.iy.theta_dot(iWEC)) = dydt_WEC(2,iWEC); % angular acceleration
    dydt(par.iy.rad(iWEC,:)) = dydt_WEC(3:end,iWEC); % radiation damping states for WEC model
end
